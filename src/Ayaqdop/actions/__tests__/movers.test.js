...
const ball = { position: [12, 9], type: ItemTypes.BAL }
...
test('move ball', () => {
  ...
  const newState = moveCore(testObjects, ItemTypes.BALL, toPosition)
  expect(newState.ball.position).toEqual(toPosition)
  expect(newState.teams[0].moves).toEqual(4)
})
test('move player', () => {
  const testObjects = JSON.parse(JSON.stringify(gameObjects))
